...
# ---------------------------------------------------------------------
st.set_page_config(
    page_title="Questionnaire DISC",
    page_icon="🧭",
    layout="wide",
)

ROOT = os.path.dirname(os.path.dirname(__file__))  # dossier EVERINSIGHT
...
# ---------------------------------------------------------------------
# ITEMS DU QUESTIONNAIRE (25 BLOCS, CHOIX FORCÉ)
# ---------------------------------------------------------------------
ITEMS = [
...
def build_radar_figure(totals, display_name: str):
    # Rayons
    rD = scale_r(totals["D"])
    rI = scale_r(totals["I"])
    rS = scale_r(totals["S"])
    ...
    for ang in [45, 135, 225, 315]:
        ax.plot([deg(ang), deg(ang)], [0, 1], color="#d9d9d9",
    ...
    thetas = [deg(45), deg(135), deg(225), deg(315), deg(45)]
    ...
    ax.plot([deg(45), deg(45)], [0, rD], color=radar_color, linewidth=1.0, alpha=0.9)
    ax.plot([deg(135), deg(135)], [0, rI], color=radar_color, linewidth=1.0, alpha=0.9)
    ax.plot([deg(225), deg(225)], [0, rS], color=radar_color, linewidth=1.0, alpha=0.9)
    ax.plot([deg(315), deg(315)], [0, rC], color=radar_color, linewidth=1.0, alpha=0.9)

    # Nom au centre : d'abord prénom, sinon email, sinon "participant"
    label = (display_name or "participant").strip()
    first_name = label.split()[0] if label else "participant"
    ax.scatter(deg(marker_angle_deg), marker_r, s=170,
               c="#D32F2F", edgecolors="none", zorder=6)
    label_r = max(0.05, marker_r - 0.08)
    ax.text(deg(marker_angle_deg), label_r, first_name,
            ha="center", va="top", fontsize=11, color="#333", zorder=7)

    ax.text(deg(45), 1.03, "D", color=COLOR["D"], ha="center", va="center",
            fontsize=14, fontweight="bold")
    ax.text(deg(135), 1.03, "I", color=COLOR["I"], ha="center", va="center",
            fontsize=14, fontweight="bold")
    ax.text(deg(225), 1.03, "S", color=COLOR["S"], ha="center", va="center",
            fontsize=14, fontweight="bold")
    ax.text(deg(315), 1.03, "C", color=COLOR["C"], ha="center", va="center",
            fontsize=14, fontweight="bold")
    ...
    return fig, dims_top2

# ---------------------------------------------------------------------
# VÉRIF : INFOS UTILISATEUR VENANT DE L'ACCUEIL
# ---------------------------------------------------------------------
...
st.markdown(
    "Ce questionnaire permet de générer un **profil DISC**. "
    "Répondez de façon spontanée, en pensant à votre manière d’agir en groupe / en projet."
)

st.info(
    f"Vous répondez au questionnaire en tant que **{display_name}** "
    f"({email})."
)

# Bouton pour tout réinitialiser (sélections des questions)
...
with st.form("disc_forced"):
    for it in ITEMS:
        ...
        if key in st.session_state and st.session_state[key] in labels:
            default_idx = labels.index(st.session_state[key])
...
missing = [qid for qid, c in answers.items() if not c]
if missing:
    st.error(
        "Vous n’avez pas répondu à tous les blocs. "
        "Blocs manquants : " + ", ".join(map(str, missing))
    )
    st.stop()
...
# DataFrame + bar chart
st.subheader("Vos résultats DISC")
...
top2 = df.iloc[:2][["Dimension", "Score"]].values.tolist()
style = "".join(dim for dim, _ in top2)  # ex. "IS"

st.success(
    f"Votre profil principal : **{DIM_LABELS[top2[0][0]][0]}** "
    f"({top2[0][0]}) · secondaire : **{DIM_LABELS[top2[1][0]][0]}** ({top2[1][0]})."
)

# ---------------------------------------------------------------------
# RADAR
# ---------------------------------------------------------------------
st.subheader("Visualisation de votre profil (radar DISC)")
fig, dims_top2 = build_radar_figure(totals, display_name)
st.pyplot(fig, clear_figure=True)

# ---------------------------------------------------------------------
# LOG JSONL (pour Mes Résultats & Plan d’action + PDF)
# ---------------------------------------------------------------------
export = {
    "ts": datetime.utcnow().isoformat() + "Z",
    "user": email,      # compat historique
    "email": email,
    "prenom": prenom,
    "nom": nom,
    "scores": totals,
...
st.caption("Vos réponses ont été enregistrées. Vous pouvez maintenant consulter "
           "l’onglet **Mes Résultats et Plan d’action**.")
